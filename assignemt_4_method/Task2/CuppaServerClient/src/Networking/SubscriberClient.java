...
import Networking.messages.client.*;
...
public class SubscriberClient {
    ...
    public void send(Message message) {
        try {
            if(message instanceof collectOrder)
            {
                Random random = new Random();
                int timesecs = 120000 + random.nextInt(180000);
    ...
    public String RandOMsTRING()
    {
        ...
        Random random = new Random();
        for (int i = 0; i <7 ; i++) {
            int n = 97+ random.nextInt(25);
            char g = (char) n;
            stringBuilder.append(g);
        }
        stringBuilder.append(Topics.customers.size());
            ...
            try {
                ...
                do {
                    ...
                    System.out.println(clientNum + " --> " + msg);
                        // Process the message.
                    if(msg instanceof SetHandle)
                    {
                        String handlestring = ((SetHandle) msg).handle;
                        if(handlestring.equals(""))
                        {
                            handlestring = RandOMsTRING();
                            ((SetHandle) msg).handle = handlestring;
                        }
                    }
                    else if(msg instanceof SendOrder)
                    {
                ...
                System.out.println(clientNum + ": Leaving customers...");
